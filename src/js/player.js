...
class Player extends Component {
  ...
  createEl() {
    ...
    const deviceClassNames = [
      ...
      .map((key) => {
        return (
          "vjs-device-" + key.substring(3).toLowerCase().replace(/\_/g, "-")
        );
      });
  ...
  crossOrigin(value) {
    ...
    if (
      ...
      log.warn(
        `crossOrigin must be null,  "anonymous" or "use-credentials", given "${value}"`
      );
      return;
    }
  ...
  loadTech_(techName, source) {
    ...
    if (
      ...
      (this.autoplay() === true && this.options_.normalizeAutoplay)
    ) {
      autoplay = false;
    }
    ...
    TECH_EVENTS_RETRIGGER.forEach((event) => {
      this.on(this.tech_, event, (e) =>
        this[`handleTech${toTitleCase(event)}_`](e)
      );
    });
    ...
    this.on(this.tech_, "fullscreenchange", (e, data) =>
      this.handleTechFullscreenChange_(e, data)
    );
    this.on(this.tech_, "fullscreenerror", (e, err) =>
      this.handleTechFullscreenError_(e, err)
    );
  ...
  updateSourceCaches_(srcObj = "") {
    ...
    const matchingSources = this.cache_.sources.filter(
      (s) => s.src && s.src === src
    );
  ...
  handleTechError_() {
    ...
    var code = error ? error.code : 3;

    if (code === 4 || code === 3) {
      // this.src(this.currentSrc());
      let currentTime = this.currentTime();
      this.currentTime(currentTime + 2);
      var seekTime = currentTime + 1;
      //this.poster('');
      // Set the source to the same one
      this.src(this.currentSrc());
      this.enableui();
      this.addClass("vjs-has-loading");
      ...
      this.one(
        "seeked",
        function () {
          // Show the video player after seeking to the desired time
          this.disableui();
          this.controlBar.show();
        }.bind(this)
      );

      // Seek to the desired time after the source has loaded
      this.one(
        "loadstart",
        function () {
          // alert("load start")
          //  this.enableui();
          // this.controlBar.hide();
          this.addClass("vjs-has-loading"); // Add a class to indicate loading
        }.bind(this)
      );

      // Hide loading spinner when the video is loaded
      this.one(
        "loadeddata",
        function () {
          // this.disableui();
          this.removeClass("vjs-has-loading"); // Remove loading class
          this.currentTime(seekTime); // Seek to the desired time after the source has loaded
          this.controlBar.show(); // Show the control bar once the video has loaded
        }.bind(this)
      );
      this.src(this.currentSrc());
      this.load();
  ...
  replay() {
    console.log("An error src occurred:", this.currentSrc());
  ...
  enableui() {
    ...
    // Add the "blank-overlay" class to the player's element
    //  this.el().classList.add("vjs-blank-overlay");
    this.addClass("vjs-blank-overlay");
  }
  ...
  disableui() {
    ...
    // Remove the "blank-overlay" class from the player's element
    this.el().classList.remove("vjs-blank-overlay");
    //this.addClass("vjs-blank-overlay");
  }
   ...
   */
  handleHotkeys(event) {
    const hotkeys = this.options_.userActions
    ...
    const {
      ...
      playPauseKey = (keydownEvent) =>
        keycode.isEventKey(keydownEvent, "k") ||
        keycode.isEventKey(keydownEvent, "Space"),
    } = hotkeys;
  ...
  selectSource(sources) {
    ...
    // that value.
    const findFirstPassingTechSourcePair = function (
      outerArray,
      innerArray,
      tester
    ) {
  ...
  autoplay(value) {
    ...
    if (
      (typeof value === "string" && /(any|play|muted)/.test(value)) ||
  ...
  loadMedia(media, ready) {
    ...
    const { artist, artwork, description, poster, src, textTracks, title } =
      this.cache_.media;
  ...
  getMedia() {
    if (!this.cache_.media) {
      ...
      const textTracks = Array.prototype.map.call(
